...
<style>
    ...
    .yellow-note {background: #FFFDE7; border-left: 4px solid #F9A825;
                  padding: 8px 12px; border-radius: 4px; font-size: 0.88rem; margin: 8px 0;}
...
if uploaded_file:
    try:
        ...
        st.success(f"✅ Loaded **{len(df)} shots** across **{df['club'].n_unique()} clubs**")
...
TARGET_TYPES = ["Proximity", "Distance Control", "Distance", "Dispersion"]

def _infer_distance(club_name: str) -> int | None:
    if not club_name:
        return None
...
def _infer_section(club_name: str, club_type: str = "") -> str:
...
if df is not None:
    ...
    st.markdown(
        '<div class="yellow-note">💡 <b>Distance (yd)</b> is auto-detected from the club name where possible. '
        'Edit any cell before generating — these values also recalculate live in the downloaded Excel.</div>',
        unsafe_allow_html=True,
    )

    if "club_configs" not in st.session_state:
        st.session_state.club_configs = [
            {
                "club": c,
                "level": 5,
                "target_type": "Proximity",
                "distance_yd": _infer_distance(c),
                "section": _infer_section(c),
                "include": True,
            }
            for c in clubs
        ]
    else:
        # Sync if clubs changed (new file uploaded)
    ...
    header_cols = st.columns([0.4, 2.5, 1.2, 1.5, 1.5, 1.8, 1.2])
    header_cols[0].markdown("**Include**")
    header_cols[1].markdown("**Club**")
    header_cols[2].markdown("**Section**")
    header_cols[3].markdown("**Target Type**")
    header_cols[4].markdown("**Distance (yd)**")
    header_cols[5].markdown("**Level (1–12)**")
    header_cols[6].markdown("**Shots**")

    for idx, cfg in enumerate(configs):
        row_cols = st.columns([0.4, 2.5, 1.2, 1.5, 1.5, 1.8, 1.2])
        cfg["include"] = row_cols[0].checkbox("", value=cfg["include"], key=f"inc_{idx}", label_visibility="collapsed")
        row_cols[1].markdown(f"**{cfg['club']}**")
        cfg["section"] = row_cols[2].selectbox("", SECTIONS, index=SECTIONS.index(cfg["section"]), key=f"sec_{idx}", label_visibility="collapsed")
        cfg["target_type"] = row_cols[3].selectbox("", TARGET_TYPES, index=TARGET_TYPES.index(cfg["target_type"]), key=f"tt_{idx}", label_visibility="collapsed")
        dist_val = row_cols[4].number_input("", min_value=0, max_value=400, value=cfg["distance_yd"] or 0, step=5, key=f"dist_{idx}", label_visibility="collapsed")
        cfg["distance_yd"] = dist_val if dist_val > 0 else None
        cfg["level"] = row_cols[5].slider("", 1, 12, value=cfg["level"], key=f"lvl_{idx}", label_visibility="collapsed")
        shot_count = len(df.filter(pl.col("club") == cfg["club"]))
        row_cols[6].markdown(f"<div style='padding-top:8px'>{shot_count} shots</div>", unsafe_allow_html=True)
...
if df is not None:
    active_configs = [c for c in st.session_state.club_configs if c.get("include", True)]
                ...
                with st.expander("☁️ Upload to Google Drive", expanded=False):
                    ...
                    if service and not isinstance(service, tuple):
                        ...
                        if folders:
                            ...
                            st.info("No folders found — file will be uploaded to root Drive.")
